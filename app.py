      ...
      <script>
        ...
        function calculateMatcherOverlap() {
            ...
            etfSet.forEach(e => {
                ...
                let matchedRows = rows.filter(r => selectedTargetStocks.some(t => t.code === r.stock));
                if (matchedRows.length > 0) {
                    let totalWeight = matchedRows.reduce((sum, r) => sum + Number(r.weight), 0);
                    let details = matchedRows.map(r => `${r.stock}(${Number(r.weight).toFixed(1)}%)`).join(', ');
                    results.push({ etf: e, name: etfNameMappingData[e] || "未知", weight: totalWeight, details: details });
                }
            });
            results.sort((a,b) => b.weight - a.weight);
            if (results.length === 0) {
                body.innerHTML = '<tr><td colspan="4" class="text-center py-4 text-muted">無任何 ETF 包含此組合</td></tr>';
            } else {
                body.innerHTML = results.map(r => `<tr>
                    ...
                    <td class="px-4 text-secondary small">${r.details}</td>
                </tr>`).join('');
            }
        }

        // 補齊其餘標籤未定義或被截斷的對應空函式，防止前端主控台報錯
