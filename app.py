...
# ==========================================
# 3. 外部即時行情 Playwright 攔截模組
# ==========================================
def fetch_wantgoo_etf_data_via_playwright(etf_filter_list):
    """
    透過 Playwright 打開玩股網 ETF 淨值走勢/折溢價首頁，
    並在背景攔截 JS 發出的網路請求，自動比對目標 ETF 代號。
    """
    if not etf_filter_list:
        return {}
    ...
    try:
        with sync_playwright() as p:
            ...
            def handle_response(response):
                # 判斷請求 URL 是否包含玩股網 ETF 行情的關鍵字路徑
                if "api/etf" in response.url or "nav-and-discount-premium" in response.url:
                    try:
                        json_res = response.json()
                        if isinstance(json_res, list):
                            for item in json_res:
                                stock_no = str(item.get("stockNo", "")).strip()
                                # 【核心邏輯】：比對是否在試算表擁有的 ETF 代號列表中
                                if stock_no and stock_no in etf_filter_list:
            ...
            # 前往網頁（這時網頁內的 JS 會被執行並自動發出 API 請求）
            page.goto(target_url, timeout=30000)
            
            # 強制等候 4 秒，確保網頁所有背景異步請求與渲染都已完成
            page.wait_for_timeout(4000)
            browser.close()
            
        return market_data
    except Exception as e:
        print(f"Playwright 攔截玩股網失敗: {e}")
    return {}
...
def fetch_backend_data_to_json():
    ...
    # 變更點：改用 Playwright 進行網頁請求攔截，傳入 all_etfs 進行精準代號比對
    wantgoo_data = fetch_wantgoo_etf_data_via_playwright(all_etfs)
...
def main():
    ...
    # 以下 HTML 模板部分完全保留你原本的漂亮樣式與前端 JS 計算邏輯
    html_template = """
    <!DOCTYPE html>
    <html>
    <head>
      ...
      <style>
        ...
        .loading-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(255,255,255,0.7); display: flex; justify-content: center; align-items: center; z-index: 9999; display: flex; }
        ...
        .rank-badge { width: 24px; height: 24px; display: inline-flex; align-items: center; justify-content: center; border-radius: 50%; font-weight: bold; font-size: 0.85rem; }
    ...
    <body>
      <nav class="navbar navbar-expand-lg navbar-dark sticky-top">
        <div class="container-fluid"><a class="navbar-brand" href="#"><i class="bi bi-cpu-fill me-2"></i>ETF 籌碼大數據監控面板</a></div>
      </nav>
      <div id="loading" class="loading-overlay"><div class="spinner-border text-primary" style="width: 3rem; height: 3rem;" role="status"><span class="visually-hidden">Loading...</span></div></div>
      <div class="container-fluid py-4 px-md-5">
        <ul class="nav nav-tabs mb-4" id="mainTabs">
          <li class="nav-item"><button class="nav-link active" id="tab-a" onclick="switchTab('content-a', 'tab-a')"><i class="bi bi-pie-chart-fill me-2"></i>單檔 ETF 籌碼與持股</button></li>
        ...
        <div id="tabsContent">
            ...
            <div class="row g-4">
              <div class="col-lg-3">
                <div class="card p-3 sticky-top" style="top: 80px; z-index: 10;">
                  ...
                  <div id="etfButtonList" class="list-group etf-list-group"><div class="text-muted text-center py-3">載入中...</div></div>
                </div>
              </div>
              <div class="col-lg-9">
                <div id="etfTitleContainer" class="etf-title-display" style="display: none;"><i class="bi bi-bookmark-star-fill me-2 text-warning"></i><span id="txtEtfCode"></span>&nbsp;&nbsp;<span id="txtEtfName" class="text-dark"></span><span id="txtUpdateDate" class="update-date-text"></span></div>
                <div id="metaContainer" class="row g-2 mb-4" style="display: none;">
                  ...
                  <div class="col-6 col-md"><div class="meta-card" style="border-left-color: #319795;"><div class="meta-label">折溢價</div><div class="meta-value" id="metaPremium">-%</div></div></div>
                  <div class="col-6 col-md"><div class="meta-card" style="border-left-color: #805ad5;"><div class="meta-label">規模</div><div class="meta-value" id="metaSize">-</div></div></div>
                ...
                <div class="row g-3">
                  <div class="col-lg-7">
                    <div class="card">
                      ...
                      <div class="table-responsive" style="max-height: 450px;"><table class="table table-hover align-middle"><thead><tr><th>股票代號</th><th>股票名稱</th><th class="text-end">持股權重</th><th class="text-end">最新持股(股)</th></tr></thead><tbody id="stockTableBody"></tbody></table></div>
                    </div>
                  </div>
                  <div class="col-lg-5">
                    <div class="card">
                      <div class="card-header text-secondary"><i class="bi bi-cash-coin me-2"></i>非股票資產項目</div>
                      <div class="table-responsive" style="max-height: 450px;"><table class="table table-hover align-middle"><thead><tr><th>資產代號</th><th>資產項目</th><th class="text-end">權重</th><th class="text-end">資產價值(股)</th></tr></thead><tbody id="assetTableBody"></tbody></table></div>
                    </div>
                  </div>
                </div>
                <div class="card p-3 mb-4 bg-light border">
                  <div class="row align-items-center g-3">
                    ...
                    <div class="col-md-5" id="customDateGroup" style="display: none;">
                      <div class="row">
                        <div class="col-6"><label class="form-label fw-bold text-secondary">舊日期 (YYYY-MM-DD)</label><input type="text" id="startDate" class="form-control" placeholder="YYYY-MM-DD"></div>
                        <div class="col-6"><label class="form-label fw-bold text-secondary">新日期 (YYYY-MM-DD)</label><input type="text" id="endDate" class="form-control" placeholder="YYYY-MM-DD"></div>
                      </div>
                    </div>
                    <div class="col-md-3 pt-4"><button class="btn btn-outline-dark w-100" onclick="refreshCurrentEtf()"><i class="bi bi-calculator me-1"></i>重新計算籌碼</button></div>
                  </div>
                  <div class="mt-2 text-muted small px-1" id="dateDisplayInfo"></div>
                </div>
                <div class="card">
                  <div class="card-header bg-dark text-white d-flex justify-content-between align-items-center"><span><i class="bi bi-lightning-charge-fill me-2 text-warning"></i>動態籌碼異動計算與連續狀態追蹤</span><span class="badge bg-secondary" id="compareDateBadge"></span></div>
                  <div class="table-responsive"><table class="table table-striped table-hover align-middle"><thead><tr><th>成分股</th><th>異動性質</th><th class="text-end">區間增減股數</th><th class="px-4">核心歷史連續買賣狀態</th></tr></thead><tbody id="changeTableBody"></tbody></table></div>
                </div>
              </div>
            </div>
          </div>
          <div class="custom-tab-content" id="content-b">
            <div class="card p-3 mb-4">
              <div class="row align-items-center g-3">
                <div class="col-md-4"><label class="form-label fw-bold text-secondary">請輸入個股代號 或 名稱 (如: 2330 或 台積電)</label><input type="text" id="stockInput" class="form-control form-control-lg" placeholder="例如: 2330 或 聯發科"></div>
                <div class="col-md-3"><label class="form-label fw-bold text-secondary">比較天數 / 範圍</label><select id="stockRangeType" class="form-select form-select-lg"><option value="1">日變動 (與前 1 筆比較)</option><option value="5">週變動 (與前 5 筆比較)</option><option value="10">與前 10 筆比較</option></select></div>
                <div class="col-md-3 pt-4"><button class="btn btn-success btn-lg w-100" onclick="searchStockDistribution()"><i class="bi bi-search me-1"></i>查詢籌碼明細</button></div>
              </div>
            </div>
            <div id="stockTrendCard" class="card mb-4" style="display: none; border-left: 5px solid #f97316;">
              <div class="card-body py-3 px-4">
                <div class="row align-items-center">
                  <div class="col-md-3"><div class="text-muted small mb-1"><i class="bi bi-hash"></i> 查詢標的</div><h3 class="fw-bold mb-0" id="trendStockHeader">-</h3></div>
                  <div class="col-md-3"><div class="text-muted small mb-1"><i class="bi bi-speedometer2"></i> 跨市場加減碼趨勢</div><h4 class="fw-bold mb-0" id="trendStockStatus">-</h4></div>
                  <div class="col-md-3"><div class="text-muted small mb-1"><i class="bi bi-diagram-3"></i> 涉及 ETF 檔數</div><h4 class="fw-bold mb-0 text-dark" id="trendStockCount">-</h4></div>
                  <div class="col-md-3 text-md-end"><div class="text-muted small mb-1">區間跨市場總變動股數</div><h3 class="fw-bold font-monospace mb-0" id="trendStockTotalVol">-</h3></div>
                </div>
              </div>
            </div>
            <div class="row g-4">
              <div class="col-lg-7">
                <div id="stockResultCard" class="card" style="display: none;">
                  <div class="card-header bg-success text-white fw-bold d-flex justify-content-between align-items-center"><span id="stockResultTitle"><i class="bi bi-arrow-left-right me-2"></i>各 ETF 區間增減股數明細</span><span class="badge bg-light text-success font-monospace" id="stockRangeBadge"></span></div>
                  <div class="table-responsive"><table class="table table-hover align-middle"><thead><tr><th>變動 ETF</th><th class="text-end">增減股數</th></tr></thead><tbody id="stockDistBody"></tbody></table></div>
                </div>
              </div>
              <div class="col-lg-5">
                <div id="stockWeightCard" class="card" style="display: none;">
                  ...
                  <div class="table-responsive"><table class="table table-hover align-middle"><thead><tr><th>ETF</th><th class="text-end">持股權重占比</th><th class="text-end">持有股數</th></tr></thead><tbody id="stockDistBody2"></tbody></table></div>
                </div>
              </div>
            </div>
          </div>
          <div class="custom-tab-content" id="content-c">
            <div class="card p-3 mb-4 bg-light">
              <div class="row align-items-center g-3">
                <div class="col-md-4"><label class="form-label fw-bold text-secondary">全市場異動比較範圍</label><select id="globalRangeType" class="form-select" onchange="toggleGlobalChanges()"><option value="1">日變動</option><option value="5">週變動</option><option value="10">月變動 (10筆)</option><option value="custom">自訂區間</option></select></div>
            ...
            <div class="card">
              ...
              <div class="table-responsive"><table class="table table-hover table-striped align-middle"><thead><tr><th>ETF</th><th>成分股</th><th>異動性質</th><th class="text-end">增減股數</th><th>連續買賣狀態</th></tr></thead><tbody id="globalTableBody"></tbody></table></div>
            </div>
          </div>
          <div class="custom-tab-content" id="content-d">
            <div class="card p-3 mb-4 bg-light">
              <div class="row align-items-center g-3">
                <div class="col-md-4"><label class="form-label fw-bold text-secondary">熱度統計比較範圍</label><select id="heatRangeType" class="form-select" onchange="toggleHeatCustomDates()"><option value="1">日變動</option><option value="5">週變動</option><option value="10">月變動 (10筆)</option><option value="custom">自訂區間</option></select></div>
            ...
            <div class="row g-4">
              <div class="col-lg-6">
                <div class="card">
                  ...
                  <div class="table-responsive"><table class="table table-hover table-striped align-middle"><thead><tr><th>排名</th><th>股票代號</th><th>股票名稱</th><th class="text-end">跨市場淨加碼(股)</th></tr></thead><tbody id="heatBuyTableBody"><tr><td colspan="4" class="text-center text-muted py-4">請點擊「生成市場熱度分析」載入數據</td></tr></tbody></table></div>
                </div>
              </div>
              <div class="col-lg-6">
                <div class="card">
                  ...
                  <div class="table-responsive"><table class="table table-hover table-striped align-middle"><thead><tr><th>排名</th><th>股票代號</th><th>股票名稱</th><th class="text-end">跨市場淨減持(股)</th></tr></thead><tbody id="heatSellTableBody"><tr><td colspan="4" class="text-center text-muted py-4">請點擊「生成市場熱度分析」載入數據</td></tr></tbody></table></div>
                </div>
              </div>
            </div>
          </div>
          <div class="custom-tab-content" id="content-e">
            <div class="card p-3 mb-4 bg-light">
              <div class="row align-items-center g-3">
                <div class="col-12"><label class="form-label fw-bold text-secondary mb-2"><i class="bi bi-check2-square me-1"></i>請選擇要比較的 ETF（可多選）</label><div id="compareEtfCheckboxes" class="d-flex flex-wrap gap-2 p-3 bg-white border rounded" style="max-height: 150px; overflow-y: auto;"></div></div>
            ...
            <div class="card">
              ...
              <div class="table-responsive"><table class="table table-hover table-striped align-middle"><thead id="compareTableHeader"><tr><th>股票代號</th><th>股票名稱</th></tr></thead><tbody id="compareTableBody"><tr><td colspan="2" class="text-center text-muted py-4">請先勾選上方 ETF 並點擊「開始交叉比較」按鈕</td></tr></tbody></table></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <script>
        ...
        function initDashboard() {
            ...
            let checkHtml = "";
            etfList.forEach(etf => {
                checkHtml += `<div class="form-check form-check-inline me-3 py-1"><input class="form-check-input etf-compare-cb" type="checkbox" value="${etf}" id="cb-${etf}" checked><label class="form-check-label fw-bold" for="cb-${etf}">${getEtfDisplayLabel(etf)}</label></div>`;
            });
            document.getElementById('compareEtfCheckboxes').innerHTML = checkHtml;

            if(etfList.length > 0) { selectEtf(etfList[0]); }
        }
        ...
        function selectEtf(etfName) {
            ...
            document.getElementById('stockTableBody').innerHTML = stocks.map(r => `<tr><td><span class="badge bg-light text-dark border font-monospace">${r.stock}</span></td><td class="fw-bold">${r.name}</td><td class="text-end text-primary fw-bold">${Number(r.weight).toFixed(2)}%</td><td class="text-end text-secondary font-monospace">${Math.round(r.volume).toLocaleString()} 股</td></tr>`).join('');
            document.getElementById('assetTableBody').innerHTML = assets.map(r => `<tr><td><span class="badge bg-light text-muted border font-monospace">${r.stock}</span></td><td><small class="text-muted">${r.name}</small></td><td class="text-end">${Number(r.weight) > 0 ? Number(r.weight).toFixed(2)+'%' : '-'}</td><td class="text-end text-secondary font-monospace">${Number(r.volume) > 0 ? Math.round(r.volume).toLocaleString() : '-'}</td></tr>`).join('');
        ...
        function renderChangeTable(etfData, sortedDates, latestDate) {
            ...
            if (sortedDates.length >= 2) {
                ...
                uniqStocks.forEach(sCode => {
                    let streakCount = 0; let currentTrend = null;
                    for (let i = sortedDates.length - 1; i > 0; i--) {
                        let dNew = sortedDates[i]; let dOld = sortedDates[i - 1];
                        ...
                        elif (dayTrend === currentTrend) { streakCount++; }
                        else { break; }
                    }
                    if (streakCount > 0 && currentTrend !== null) { trendMap[sCode] = `連${currentTrend} ${streakCount} 日`; }
                    else { trendMap[sCode] = "無變動"; }
                });
            }
            ...
            currentStocks.forEach(r => {
                ...
                if (diff !== 0) {
                    ...
                    if (nature === "新增") { badge = `<span class="badge-nature-new">${nature}</span>`; dStyle = "color:#ea580c;"; }
                    elif (nature === "增加") { badge = `<span class="badge-nature-up">${nature}</span>`; dStyle = "color:#dc2626;"; }
                    else { badge = `<span class="badge-nature-down">${nature}</span>`; dStyle = "color:#0f766e;"; }
                    ...
                    let rowHtml = `<tr><td class="fw-bold">${r.stock} <span class="text-muted small fw-normal ms-2">${r.name}</span></td><td>${badge}</td><td class="text-end fw-bold font-monospace" style="${dStyle}">${diff > 0 ? '+' : ''}${Math.round(diff).toLocaleString()} 股</td><td class="px-4">${trendHtml}</td></tr>`;
                    if (nature === "新增") htmlNew += rowHtml;
                    elif (nature === "增加") htmlAdd += rowHtml;
                    elif (nature === "減少") htmlSub += rowHtml;
                }
            });
            ...
            compRows.forEach(r => {
                if (isNormalStock(r.stock, r.name)) {
                    ...
                    if (!isStillExist && r.volume > 0) {
                        ...
                        htmlDel += `<tr><td class="fw-bold">${r.stock} <span class="text-muted small fw-normal ms-2">${r.name}</span></td><td>${badge}</td><td class="text-end fw-bold font-monospace" style="${dStyle}">${Math.round(diff).toLocaleString()} 股</td><td class="px-4">${trendHtml}</td></tr>`;
                    }
                }
            });
        ...
        function searchStockDistribution() {
            ...
            if (tickerMappingData[target]) { targetName = tickerMappingData[target]; }
            else { for (let key in tickerMappingData) { if (tickerMappingData[key] === target) { targetCode = key; break; } } }

            let latestMatches = globalRawData.filter(d => d.date === latestDate && (d.stock === targetCode || d.name === targetName));
            if (latestMatches.length > 0) { targetCode = latestMatches[0].stock; targetName = latestMatches[0].name; }
        ...
        function loadGlobalChanges() {
            ...
            let body = document.getElementById('globalTableBody'); body.innerHTML = ""; let anyChange = false;
            ...
            etfList.forEach(eCode => {
                ...
                curStocks.forEach(r => {
                    ...
                    let diff = r.volume - oldVol;
                    if(diff !== 0) {
                        anyChange = true; let bClass = diff > 0 ? "badge-nature-up" : "badge-nature-down";
        ...
        function generateComparison() {
            ...
            let stockMap = {};
            globalRawData.forEach(r => { if(r.date === latestDate && checkedCbs.includes(r.etf) && isNormalStock(r.stock, r.name)) { stockMap[r.stock] = r.name; } });
