...
def fetch_ticker_map():
    ...
    try:
        ...
        # 🛡️ 方案二改造：改用 get_all_values() 手動解析對照表，杜絕重複空欄位的 error
        ...
        # 備份相容查找
        if ticker_idx == -1 or name_idx == -1:
            for idx, h in enumerate(headers):
                if "代號" in h or "Ticker" in h or "Symbol" in h:
                    ticker_idx = idx
                if "名稱" in h or "Name" in h:
                    name_idx = idx
...
def fetch_etf_name_map():
    ...
    try:
        ...
        etf_name_map = {}
        for row in raw[1:]:
            if len(row) > max(etf_idx, name_idx):
                e = row[etf_idx].strip()
                n = row[name_idx].strip()
                if e:
                    etf_name_map[e] = n
